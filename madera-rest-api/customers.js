...
module.exports = {
  get(req, res, next) {
    if (req.params.id) {
      connection.query('SELECT * from customers where id=?', req.params.id, (err, results, fields) => {
  ...
  add(req, res, next) {
    ...
    connection.query('INSERT INTO customers (lastName, firstName, ref, address) values (?, ?, ?, ?)',
      [
        req.body.lastName,
        req.body.firstName,
        req.body.ref,
